...
config = rs.config()
bag_path = "/home/guilh/Data_Vineyard_autonomous_robot/Vinha-09-04/row1.bag"
rs.config.enable_device_from_file(config, bag_path, repeat_playback = False)
device = config.resolve(pipeline).get_device().as_playback()
device.as_playback().set_real_time( True )
...
config.enable_stream(other_stream)
