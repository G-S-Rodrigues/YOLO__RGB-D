...
def get_input_data(img_file):
    ...
    Returns:
        tuple: A tuple containing the image, ground truth bounding boxes.
    ...
    get_key_points = get_kp_list(lab_pth, CONSTANTS['CLASSES'])
    return image, get_key_points, aug_file_name
...
def get_kp_list(inp_lab_pth, classes):
    """
    Reads YOLO format labels from a file and returns polygons segmentation information.
    ...
    album_key_points_lists = get_album_bb_lists("\n".join(lines), classes) if len(lines) > 1 else [get_album_bb_list("\n".join(lines), classes)]
    
    return album_key_points_lists
...
def single_obj_bb_yolo_conversion(transformed_bboxes, class_names):
    ...
    if transformed_bboxes:
        ...
        bboxes = list(transformed_bboxes)[2:-1]
...
def get_augmented_results(image, keypoints):
    """
    ...
    transform = A.Compose([
        A.SafeRotate(limit=[-15, 15],value=0,always_apply=True,border_mode=cv2.BORDER_CONSTANT),
    ...
    transformed_keypoints = [i[:-1] for i in sorted(transformed['keypoints'], key=lambda x: x[2])]
    transformed_image = transformed['image']
    
    return transformed_image, transformed_keypoints, aug
